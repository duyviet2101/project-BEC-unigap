...
import vn.unigap.common.logger.telegram.LoggerTelegramBot;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    final LoggerTelegramBot loggerTelegramBot;

    @Autowired
    public ApiExceptionHandler(LoggerTelegramBot loggerTelegramBot) {
        this.loggerTelegramBot = loggerTelegramBot;
    }
    ...
    private void captureException(Exception e, HttpStatusCode statusCode) {
        ...
        String messageTelegram = formatMessage(event);
        loggerTelegramBot.log(messageTelegram);
    }

    private String formatMessage(SentryEvent event) {
        Instant instant = Instant.ofEpochMilli(event.getTimestamp().getTime());
        String timestamp = LocalDateTime.ofInstant(instant, ZoneId.of("Asia/Ho_Chi_Minh"))
        ...
        return String.format("""
                ...
                Transaction: %s
                Timestamp: %s
                """, event.getEventId(), event.getLevel(), Objects.requireNonNull(event.getThrowable()).getMessage(),
                event.getTransaction(), timestamp);
    }
}